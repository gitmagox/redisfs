...
TestDecompress (CuTest * tc)
{
   ...
   * Attempt the compression.
   */
  int ret = compress2(compressed, &compressed_len ,(void*) input, input_len,Z_BEST_SPEED);
   ...
   */
  ret = uncompress( decompressed,  &decompressed_len, (void*)compressed, compressed_len );
